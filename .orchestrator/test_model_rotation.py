...
def test_generic_provider_quota_markers_still_classified():
    ...
    for reason in ("Status: 402 credit balance is too low", "You have no quota", "[API Error: Helper OAuth free tier quota exceeded.]", "free tier quota exceeded"):
...
def test_cloud_run_quota_exceeded_stays_terminal_not_provider_quota():
    ...
    for provider in ("codex", "claude", "antigravity5", "copilot", "gemini"):
        assert sv.classify_worker_failure(CFG, {"provider": provider}, "Cloud Run API quota exceeded")["kind"] == "terminal"
        result = sv.classify_worker_failure(CFG, {"provider": provider}, "429 Quota exceeded for quota metric 'Cloud Run API quota exceeded'")
