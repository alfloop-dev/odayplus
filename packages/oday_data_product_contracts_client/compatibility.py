"""Fail closed when the released product package drifts away from the pin.

This is the gate the acceptance criterion asks for: an incompatible product schema
must fail CI rather than degrade silently at runtime. Every check below compares
the *released* catalog against `config/oday_data_product_contracts.toml`, so the
failure names the contract that moved and how.
"""
    ...
    def raise_for_drift(self) -> None:
        if self.drifts:
            ...
            raise IncompatibleContractError(
                "the released ODay data-platform product package is not compatible "
                f"with config/oday_data_product_contracts.toml:\n  - {detail}"
            )


def _release_identity_drifts(release: ProductRelease) -> Iterator[ContractDrift]:
    pin = release.pin
...
def _contract_drifts(
    ...
    if entry is None:
        yield ContractDrift(
            ...
            reason="contract is no longer published by the product release",
...
def check_release(release: ProductRelease | None = None) -> CompatibilityReport:
...
def verify_release(release: ProductRelease | None = None) -> CompatibilityReport:
