"""Read `config/oday_data_product_contracts.toml`, the product release pin.
...
DEFAULT_PIN_PATH = REPO_ROOT / "config" / "oday_data_product_contracts.toml"
...
class ReleasePin:
    """Identity of the released product package this consumer is pinned to."""
...
@dataclass(frozen=True, slots=True)
class ContractPin:
    """The parsed `config/oday_data_product_contracts.toml` document."""
...
def load_pin(path: Path | str | None = None) -> ContractPin:
    ...
    try:
        ...
        raise PinError(f"{pin_path}: product contract pin is missing") from exc
    except tomllib.TOMLDecodeError as exc:
        raise PinError(f"{pin_path}: product contract pin is not valid TOML: {exc}") from exc
    ...
    for index, entry in enumerate(entries):
        ...
        pinned = PinnedContract(
            ...
            root_model=_optional_text(entry, "root_model"),
        )
