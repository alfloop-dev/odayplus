"""Expose the exact pinned product version to runtime callers.

`product_version()` is what an API health route, a worker startup log, or an
...
the released catalog still satisfies the pin, so "which product contracts are we on?"
and "is that release still the one we validated against?" cannot disagree.
"""
...
from .release import ProductRelease, load_release
...
class ProductVersion:
    """The exact released product package this process is running against."""
...
def _version_of(release: ProductRelease) -> ProductVersion:
    pin = release.pin
...
@lru_cache(maxsize=1)
def _verified_release() -> ProductRelease:
...
def product_version() -> ProductVersion:
    """Return the verified product version. Raises if the pin no longer holds."""
    return _version_of(_verified_release())


def product_contracts() -> tuple[ContractInfo, ...]:
...
def diagnostics() -> dict[str, Any]:
    ...
    return {
        "product": version.as_dict(),
        "contracts": [contract.as_dict() for contract in product_contracts()],
    }
