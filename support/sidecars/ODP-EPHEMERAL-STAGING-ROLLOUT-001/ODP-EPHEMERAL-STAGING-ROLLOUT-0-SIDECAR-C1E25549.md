...
| **Parent Task Status** | `blocked`（`waiting_for: Human/Ops, dev live rollout remediation, runtime release lifecycle`） |
...
`ODP-EPHEMERAL-STAGING-ROLLOUT-001` 目前仍不可進入 staging admission。阻塞來自三個尚未完成的前置條件：
...
3. `ODP-RUNTIME-RELEASE-STAGING-LIFECYCLE-INTEGRATION-001`：只有唯一 Runtime Release 已直接整合 release-scoped lifecycle 後，才能建立 staging rehearsal。
...
| `ODP-RUNTIME-RELEASE-STAGING-LIFECYCLE-INTEGRATION-001` | Engine / Rollout | 待整合 | 將 release-scoped lifecycle 與唯一 Runtime Release 直接整合，確保 staging rehearsal 可安全受控啟動與銷毀。 |
...
flowchart TD
    ...
    LIFECYCLE["ODP-RUNTIME-RELEASE-STAGING-LIFECYCLE-INTEGRATION-001<br/>[待整合]"]
...
| External data boundary | `python3 scripts/validate_external_data_boundary.py` | `external-data boundary: OK` | 外部資料邊界分類完整（2787 個檔案已分類）；rehearsal 仍須維持 provider disabled。 |
...
1. Parent task 維持 `blocked` 與 `NO-GO`，等待 `HUMAN-GCP-WEB-OAUTH-CLIENTS-001`、`ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` 與 `ODP-RUNTIME-RELEASE-STAGING-LIFECYCLE-INTEGRATION-001`。
