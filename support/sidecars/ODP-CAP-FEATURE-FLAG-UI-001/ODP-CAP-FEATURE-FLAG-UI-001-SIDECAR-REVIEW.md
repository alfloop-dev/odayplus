...
| `apps/web/features/operator/GovernanceWorkspace.tsx` | Operator route | Surfaces the workspace as the `featureFlags` tab of the Govern workspace, reachable at `/operator?ws=govern`. The web app keeps exactly three executable pages (`franchisee`, `intake/[intakeId]`, `operator`), asserted by `src/app/__tests__/productionRoutes.test.ts`; sub-screens are workspaces and tabs, not new routes. |
