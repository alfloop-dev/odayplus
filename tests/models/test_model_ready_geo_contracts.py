...
def test_sitescore_sql_has_fixed_mature_horizon_and_strict_prior_features() -> None:
    ...
    assert "count(DISTINCT lineage.canonical_table) = 3" in body
    assert "lineage.canonical_table = 'core.address_locations'" in body
...
def test_supported_geo_contracts_are_active_only_after_views_are_declared() -> None:
    ...
    assert (
        sql.index("CREATE OR REPLACE VIEW model_ready.heatzone_training_view")
        < registry_offset
    )
