...
def test_tw_only_residency_deny_by_default() -> None:
    # Setup document store Mock for resolving residency mode
    tenants = {
        "tenant-tw": {"residency_mode": "TW_ONLY"},
    }
    ...
    # 1. Blocked bucket names that should fail (repro of Frankfurt, Singapore, Japan)
    for bad_bucket in ("odp-snapshots-frankfurt", "acme-prod-singapore", "snapshots-japan", "any-random-bucket"):
        with pytest.raises(ResidencyDeniedError):
            service.create_snapshot(
                tenant_id="tenant-tw",
                intake_id="IN-2",
                source_id="src-1",
                raw_data=b"restricted data",
                original_url="http://src1.com",
                canonical_url="http://src1.com",
                media_type="text/html",
                capture_method="SERVER_RETRIEVAL",
...
def test_legal_hold_and_export_and_lineage() -> None:
    ...
    # Create first snapshot (version 1)
    snap_id_1 = service.create_snapshot(
        tenant_id="tenant-tw",
        intake_id="IN-100",
        source_id="src-1",
        raw_data=b"v1 data",
        ...
        captured_at=datetime.now(UTC),
        bucket="snapshots-taiwan",
        legal_hold=True,  # Put on legal hold
    )

    # Create second snapshot (version 2 correction)
    snap_id_2 = service.create_snapshot(
        tenant_id="tenant-tw",
        intake_id="IN-100",
        source_id="src-1",
        raw_data=b"v2 data",
    ...
    # 1. Enforce legal hold on delete: deletion must fail
    from modules.listing.domain.intake_states import DomainValidationError
    with pytest.raises(DomainValidationError) as exc:
        service.delete_snapshot("tenant-tw", snap_id_1)
    assert "active legal hold" in str(exc.value)

    # Deletion of non-held snapshot should succeed
    service.delete_snapshot("tenant-tw", snap_id_2)
    assert service.get_snapshot("tenant-tw", snap_id_2) is None

    # 2. Enforce residency / export restrictions on export
    # Exporting snap_id_1 to non-TW destination should fail with RESIDENCY_DENIED
    with pytest.raises(DomainValidationError) as exc:
        service.get_snapshot_for_export("tenant-tw", snap_id_1, destination_residency="APPROVED_APAC_DR")
    assert "RESIDENCY_DENIED" in str(exc.value)

    # Exporting snap_id_1 to TW_ONLY should fail with LEGAL_HOLD_CONFLICT due to active legal hold
    with pytest.raises(DomainValidationError) as exc:
        service.get_snapshot_for_export("tenant-tw", snap_id_1, destination_residency="TW_ONLY")
    assert "LEGAL_HOLD_CONFLICT" in str(exc.value)

    # 3. Check lineage retrieval
    lineage = service.get_correction_lineage("tenant-tw", "IN-100")
    # snap_id_2 was deleted, so we should see snap_id_1
    assert len(lineage) == 1
    assert lineage[0]["source_snapshot_id"] == snap_id_1
...
def test_postgres_rls_isolation_on_snapshots(intake_db) -> None:
    ...
    with intake_db.connect(autocommit=False) as conn:
        cur = conn.cursor()
        cur.execute(
            """
        ...
        # Create a non-superuser role to test RLS because superusers always bypass RLS
        cur.execute("DROP ROLE IF EXISTS snapshot_rls_test_role")
        cur.execute("CREATE ROLE snapshot_rls_test_role NOSUPERUSER")
        for schema in ("intake", "identity", "expansion", "workflow", "audit"):
            cur.execute(f"GRANT USAGE ON SCHEMA {schema} TO snapshot_rls_test_role")
            cur.execute(f"GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA {schema} TO snapshot_rls_test_role")
        ...
        cur.execute(
            ...
            """,
            (
                "00000000-0000-0000-0000-000000000001", tenant_a, source_id, "https://rls.com/a", "https://rls.com/a", "SUBMITTED", 1,
        ...
        cur.execute(
            ...
            """,
            (
                "00000000-0000-0000-0000-000000000002", tenant_b, source_id, "https://rls.com/b", "https://rls.com/b", "SUBMITTED", 1,
                "00000000-0000-0000-0000-000000000000", "URL", "00000000-0000-0000-0000-000000000000"
            )
        )
    ...
    with intake_db.connect(autocommit=False) as conn:
        cur = conn.cursor()
        cur.execute("SET ROLE snapshot_rls_test_role")
        cur.execute("SELECT set_config('app.tenant_id', %s, true)", (tenant_a,))
