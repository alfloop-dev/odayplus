...
def test_verified_email_mapping_supplies_platform_scope_for_generic_oidc(config, key):
    ...
    outcome = _boundary(mapped).authenticate(
        Credentials(bearer_token=encode_compact_jwt(claims, key)),
        now=NOW,
    )
    ...
    assert outcome.principal.roles == frozenset(
        {Role.OPERATIONS_MANAGER, Role.SITE_REVIEWER}
    )
    assert outcome.principal.scope.region_ids == frozenset({"north"})


def test_unverified_email_cannot_select_principal_mapping(config, key):
    mapped = AuthBoundaryConfig(
        issuer=config.issuer,
        audiences=config.audiences,
        signing_keys=config.signing_keys,
...
def test_configured_principal_map_does_not_fall_back_to_token_authorization(
    config, key
):
    mapped = AuthBoundaryConfig(
        issuer=config.issuer,
    ...
    assert outcome.principal.roles == frozenset()
    assert outcome.principal.tenant_id is None
    assert outcome.principal.scope.region_ids == frozenset()


def test_principal_map_missing_fields_do_not_fall_back_to_token_claims(config, key):
    mapped = AuthBoundaryConfig(
        issuer=config.issuer,
        audiences=config.audiences,
        signing_keys=config.signing_keys,
    ...
    assert outcome.principal.tenant_id == "tenant-live"
    assert outcome.principal.scope.region_ids == frozenset()
    assert outcome.principal.scope.clearance == Scope().clearance
...
def test_config_from_env_reads_principal_mapping():
    cfg = config_from_env(
        {
    ...
    assert cfg.principal_mappings["operator@example.com"]["tenant_id"] == "tenant-live"
    assert cfg.principal_mapping_declared is True


@pytest.mark.parametrize("value", ["{not-json", "[]", "{}"])
def test_declared_empty_or_invalid_principal_mapping_remains_authoritative(value):
    cfg = config_from_env({"ODP_AUTH_PRINCIPAL_MAP": value})

    assert cfg.principal_mapping_declared is True
    assert cfg.principal_mappings == {}


def test_declared_invalid_principal_mapping_never_falls_back_to_token_claims(key):
    cfg = config_from_env(
        {
            "ODP_AUTH_ISSUER": ISSUER,
            "ODP_AUTH_AUDIENCES": AUDIENCE,
            "ODP_AUTH_HS256_KEYS": f"{key.kid}:{key.secret.decode()}",
            "ODP_AUTH_PRINCIPAL_MAP": "{not-json",
        }
    )
    claims = _claims(
        roles=["platform_admin"],
