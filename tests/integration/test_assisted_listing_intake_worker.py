...
def test_async_intake_worker_happy_path(db_path) -> None:
    ...
    try:
        ...
        bundle.job_queue.update_status(job.job_id, JobStatus.SUCCEEDED, expected_version=latest_job.version, fence_token=latest_job.fence_token)
        
        # Retrieve the intake record after processing
        get_resp = client.get(f"/api/v1/operator/network-listings/intake/{data['id']}", headers=HEADERS)
