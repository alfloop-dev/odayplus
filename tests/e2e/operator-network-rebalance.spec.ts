...
test.describe("ODP-OC-R4-008 Network Rebalance", () => {
  ...
  test("AVM + NetPlan workflow persists selected scenario and creates Govern approval without execution", async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId("rebalance-avm-RB-801")).toContainText(
      "service output",
    );
    await expect(page.getByTestId("rebalance-avm-RB-801")).toContainText(
      "avm-rebalance-income-market-v1.0.0",
    );
    await expect(page.getByTestId("rebalance-avm-RB-801")).toContainText(
      "AVM-SNAP-20260714-0600",
    );
    await expect(page.getByTestId("rebalance-primary-action")).toContainText(
      "建立 NetPlan Review",
    );
    ...
    await expect(page.getByTestId("rebalance-scenario-keep")).toContainText(
      "Keep / Improve",
    );
    await expect(page.getByTestId("rebalance-scenario-move")).toContainText(
      "Move (移轉新址)",
    );
    await expect(page.getByTestId("rebalance-scenario-move")).toContainText(
      "系統建議",
    );
    await expect(page.getByTestId("rebalance-scenario-exit")).toContainText(
      "Exit (關店止損)",
    );
    await expect(page.getByTestId("rebalance-scenario-move")).toContainText(
