...
- **處置狀態**：`VERIFIED`
- **實作證據 (Evidence)**: `modules/avm/domain/valuation.py::value_store`
...
- **驗收標準 (Acceptance Criteria)**: `modules/avm/tests/test_avm_depreciation_contract.py` 的八條契約規格（C-1 基數判別、C-2 輸入欄位、C-3 直線折舊計算、C-4 evidence 與版本欄位、C-5 缺席 fail-closed、L-1／L-2 舊估值卡、L-4 校準分版本、R-1 逐位元回滾）已全數實作並通過測試。
- **工程處置 (Engineering Disposition)**:
  採 **AVM-specific model**，依 straight-line 計算公式完成折舊扣減與基數判別（`original_cost` 折舊、`appraised_fair_value` 不重複折舊），輸出攜帶 `depreciation_version` 與 `depreciation_applied` 標記，舊卡透過 `rehydrate_legacy_valuation_card` 賦予 legacy 版本標記，校準計算依折舊版本獨立統計分析。
- **回歸測試**: `tests/governance/test_avm001_disposition.py` 與 `modules/avm/tests/test_avm_depreciation_contract.py`。
