...
applied by `scripts/apply_branch_protection.py`.
...
`branch_overrides.dev.strict = false`; `main` has no queue and keeps
`strict = true`.
...
```bash
python3 scripts/apply_branch_protection.py --verify-only
```
...
```bash
python3 scripts/apply_branch_protection.py --disable-merge-queue
```
...
* `.orchestrator/auto_merge_green_prs.py` probes
  `repository.mergeQueue(branch:"dev")` on each run and picks
  `gh pr merge --merge --auto` (enqueue) or `gh pr merge --merge` (direct)
  accordingly, so it follows the rollback without a code change.
...
| `gh pr merge` fails with a merge-queue error | a caller is still merging directly | that caller needs the `--auto` enqueue path, as in `auto_merge_green_prs.py` |
| Queue merges nothing while PRs sit "ready to merge" | nothing is enqueueing them | `.orchestrator/auto-merge-guard.sh` / `auto_merge_green_prs.py` guard loop is not running |
