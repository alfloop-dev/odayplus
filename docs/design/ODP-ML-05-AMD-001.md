...
上述變更由 PR #1095 交付（已併入 `dev`，merge commit `35a28741`）。截至 2026-09-01，`ADR-0004` 本身（PR #1090）**尚未併入 `dev`**，請以 `task/ODP-EVIDENCE-LEVEL-ADR-001` 查核，不要以 `dev` 查核。此為快照，併入狀態以 `gh pr view <PR>` 的當下結果為準。
