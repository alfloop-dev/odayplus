...
`delivery_toolchain/governance/validate_assisted_listing_intake_design.py` parses and compares all three registers and fails closed on any membership, precedence, or order difference. `delivery_toolchain/openapi/build_validate_assisted_listing_intake.py` reads the OpenAPI order from the review manifest instead of maintaining a separate default list.
