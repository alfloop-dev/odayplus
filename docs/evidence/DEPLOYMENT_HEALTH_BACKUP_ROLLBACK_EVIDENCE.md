...
| Remote staging rollout | blocked | `.github/workflows/deploy-dev.yml` (Runtime Release) now fails closed through `delivery_toolchain/e2e/check_remote_staging_proof.py`; staging host/API URL/secret owner and backing services are still not configured |
