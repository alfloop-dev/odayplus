...
2. **Staging Rollout & Verification**:
   - Integrated check-remote-staging-proof tool (`delivery_toolchain/e2e/check_remote_staging_proof.py`) in `.github/workflows/deploy-staging.yml` which fails closed when variables are missing.
...
3. **Backup, Restore & Rollback E2E**:
   - Runs `delivery_toolchain/e2e/verify_deployment_health_backup_rollback.py` as part of Dev deployment to verify database state preservation and recovery.
...
- Static release gate check script (`delivery_toolchain/e2e/check_product_release_gate.py`) returns success, verifying presence of all required release deliverables and artifacts.
