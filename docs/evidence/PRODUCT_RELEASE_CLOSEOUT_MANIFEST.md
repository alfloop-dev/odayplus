...
| External proof issue sync | `python3 scripts/e2e/check_external_proof_issue_sync.py` verifies live GitHub issues #132-#138 still carry the queue-defined fleet routing labels, pickup commands, release authority, and completion boundaries; add `--require-assignees` once named GitHub assignees are available | prepared, live GitHub check |
