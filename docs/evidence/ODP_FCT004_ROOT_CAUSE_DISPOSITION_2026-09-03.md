...
本任務完成以下工作：
1. **全樹 Writer / Reader 證明**：完成跨 Python domain、ForecastOps、API、Ingestion contracts、TypeScript schemas、dbt pipelines 及 PostgreSQL DDL 的完整 lineage 查證，證實全系統完全不存在自動化根因推導（automated root cause deduction）的生產者或消費者。
...
3. **正式處置為 `RESERVED (unproduced)`**：
   ...
   - 更新 `shared/domain/models.py` 與 `packages/schemas/canonical/index.ts`，明確標記 `@reserved` / `RESERVED (unproduced)`，消解 API 與 TS 契約暗示該能力已存在的誤導。
   - 新增 forward migration `000017_work_orders_root_cause_disposition.sql` 與 Alembic revision `0011_work_orders_root_cause_disposition.py`，於資料庫層級記錄 column comment 與保留語意，並具備完整 rollback 機制。
...
全樹掃描 `root_cause`、`WorkOrder` 及 `FCT-004` 相關實體之結果如下：

| 層級 / 檔案 | 宣告形式 | Production Writer 存在性 | Production Reader 存在性 | 說明 |
|---|---|:---:|:---:|---|
| `shared/domain/models.py::WorkOrder` | `root_cause: str \| None = None` | ❌ 無 | ❌ 無 | 僅在單元/契約測試中建構，業務模組無呼叫 |
| `packages/schemas/canonical/index.ts::WorkOrder` | `root_cause: string \| null` | ❌ 無 | ❌ 無 | 前端/API client 無任何邏輯讀寫 |
| `infra/db/migrations/000001_baseline_canonical_schema.sql` | `core.work_orders.root_cause TEXT` | ❌ 無 | ❌ 無 | 無任何 DAO、SQL query 或 dbt 模型讀寫該欄位 |
...
| `packages/domain-types/` (`RootCauseEvidenceCardContract`) | UI presentation contract | ❌ 無後端產出 | ⚠️ UI component (純 contract) | 介面規範存在，但後端 Alert API 僅提供原始 `evidence_json` |
...
3. **選項 C：標記為 `RESERVED (unproduced)`（採納）**
   - 保留資料相容性，同時在所有型別、API 契約、資料庫註解與治理清單中明示「目前版本無自動化生產者」。
...
3. **Database Migration (`infra/db/migrations/`)**
   - `000017_work_orders_root_cause_disposition.sql`：執行 `COMMENT ON COLUMN core.work_orders.root_cause IS 'RESERVED: No automated producer exists in current release (ODP-FR-FCT-004). Owner: ForecastOps/Platform; Target: Wave 5+';`
   - `versions/0011_work_orders_root_cause_disposition.py`：Alembic 遷移腳本，支援 `upgrade` 與 `downgrade` 回滾。
4. **Governance Registry (`delivery_toolchain/governance/set_valued_requirements.json`)**
   - 登錄 `ODP-FR-FCT-004`，4 個 members（3 個 satisfied、`ROOT_CAUSE_CANDIDATE` 記為 `absent` 並附完整 disposition note）。
5. **Contract & Regression Tests (`tests/contract/`, `tests/ops/`)**
   - 新增 `tests/contract/test_root_cause_contract_disposition.py`（6 項驗證通過）。
