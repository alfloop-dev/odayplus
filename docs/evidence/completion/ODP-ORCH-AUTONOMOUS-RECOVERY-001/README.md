...
- Independent review round 3 (Codex2, board event `reopen` at `2026-09-24T09:28:57Z`) returned
  one P2 finding (R4) regarding empty porcelain status collision on `status_failed`. Fixed in
  `worker_workspace.py` and `sealed_owner_continuation_allowed` with shipped regression tests.
...
| R4 (P2) | `_interrupted_merge_worktree_fingerprint` only iterated over `inspection.entries` without binding `inspection.kind`. When `git status` failed (`status_failed`) with `entries=()`, it returned `sha256(b"")`, which collided with clean empty-porcelain merge status (`clean` with `entries=()`). In `sealed_owner_continuation_allowed`, an in-progress clean merge with a status reading failure and dirty drift after seal was erroneously allowed as an exact state. | Real clean merge with divergent allow-empty commits sealed; setting invalid status config and adding dirty `README.md` caused `git status` exit 128 (`inspection.kind=status_failed`), but `direct_allowed=true` and `lease_ok=true` erroneously accepted continuation. | `_interrupted_merge_worktree_fingerprint` explicitly binds `inspection.kind` and returns `"status_failed"` on status reading failure; `sealed_owner_continuation_allowed` explicitly fails closed on `status_failed`; `_quarantine_and_preserve_dead_worker` refuses to create an `interrupted_merge` continuation seal on status failure. |
...
`test_interrupted_merge_seal_rejects_symlink_target_drift` (symlink target drift rejection),
`test_interrupted_merge_seal_rejects_hardlink_byte_drift` (hardlink byte drift rejection),
`test_interrupted_merge_seal_rejects_empty_porcelain_merge_status_failure_and_dirty_drift` (empty porcelain status_failed and dirty drift fail-closed), and
`test_interrupted_merge_seal_refuses_when_status_fails_at_seal_time` (seal-time status failure refusal). Two pre-existing mock-only
