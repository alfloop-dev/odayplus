...
- **實作處置**:
  ...
  - `infra/terraform/validate_contract.py` 增設斷言，確保 `main.tf` 不投射 `ODP_PRODUCTION_PROVIDER_IDS` 或定義 provider secret 名稱清單。
...
- **實作處置**:
  ...
  - `infra/terraform/validate_contract.py` 驗證防火牆規則與私有連線設定。
...
- **實作處置**:
  ...
  - 執行 `python3 scripts/validate_external_data_boundary.py`，全庫 2699 個檔案分類完整（0 unclassified）、32 個凍結檔案無未經授權異動、外部邊界檢核完整通過（`external-data boundary: OK`）。
...
- **實作處置**:
  - 在 `tests/ops/test_cloud_run_live_deployment.py` 新增負向與不變性測試：
    - `test_preflight_rejects_external_provider_live_mode`: 驗證若有人嘗試注入 `ODP_EXTERNAL_PROVIDER_MODE=live`，預檢立即失敗。
  ...
  - 更新既有測試對 workflow、deploy script 及 Terraform HCL 檔案中 provider 關閉狀態的驗證斷言。
...
| 檢查項目 / 測試套件 | 執行命令 | 結果 |
|-------------------|---------|------|
| 全庫 External Data 邊界分類 | `python3 scripts/validate_external_data_boundary.py` | **2699/2699 分類完整，32 凍結檔案完整，0 違規 (PASSED)** |
...
| Terraform 契約單元測試 | `uv run python -m unittest infra/terraform/tests/test_contract.py` | **4/4 全部通過 (OK)** |
