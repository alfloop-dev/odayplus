...
| 5 | Desktop and constrained-width screenshots are compared with the archived interactive HTML for every changed R4 surface; unresolved visual differences block approval. | ✅ | Archived package-6 HTML rendered (desktop + 768 constrained) and matched against the delivered app at both widths for the entry cards, five-step builder, lifecycle, and outcome surfaces. No unresolved difference (constrained reflow is additive). `visual-parity.md §3` + paired `archived-growth-*.png` / `growth-impl-*.png`. |
