...
1. **Release governance configs** — `infra/assisted-listing-intake/`
   ...
   `rollback_triggers.yaml`, `README.md`). All production flags are **disabled**
   and §12 owner approvals are recorded as **pending**; the harness fails closed
   on any drift from this state.
2. **Release drill harness** — `scripts/release/assisted_listing_intake/`
   ...
   `tests/ops/test_assisted_listing_intake_release.py` (17 tests).
...
| canary | `canary.json` | PASS — write-canary units 1–2 executed, units 3+ **BLOCKED** as required |
...
CI=1 npx playwright test tests/e2e/operator-assisted-listing-intake.spec.ts --reporter=json
  → 7 passed + 1 flaky (passed on retry), unexpected=0
  → CI=1 is required: the score-failure fault control in
    apps/api/app/routes/listings.py is gated on CI=1 and 403s otherwise.
uv run pytest tests/ops/test_assisted_listing_intake_release.py -q   → 17 passed
...
- **Live staging / production canary units 3+**: no live staging environment is
  ...
  staging vars/secrets absent). The canary ladder therefore executes only the
  non-production units and records units 3+ as BLOCKED; the cutover phase
  verifies this and stays fail-closed.
