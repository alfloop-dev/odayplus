...
- External proof handbacks must use
  ...
  `python3 delivery_toolchain/e2e/check_external_proof_handback_template.py` before
...
- Product Validation must keep handback intake state in
  ...
  `python3 delivery_toolchain/e2e/check_external_proof_handback_status_board.py` so the
...
- Update that board with
  `python3 delivery_toolchain/e2e/update_external_proof_handback_status_board.py`, not by
...
- External proof GitHub issue handoff must remain synced with queue routing.
  ...
  `python3 delivery_toolchain/e2e/sync_external_proof_fleet_issues.py --release-sha "$(gh pr view 82 --json headRefOid --jq .headRefOid)" --apply`
  to refresh #132-#138 issue bodies and pickup comments from
  `docs/evidence/PRODUCT_EXTERNAL_PROOF_CLOSEOUT_QUEUE.json`.
  Run `python3 delivery_toolchain/e2e/check_external_proof_issue_sync.py --require-assignees`
...
- External proof live blocker state must match Product Validation intake
  status. Run `python3 delivery_toolchain/e2e/check_external_proof_live_blockers.py --require-assignees`
...
- External proof escalation comments should use the standard renderer. Run
  ...
  `python3 delivery_toolchain/e2e/sync_external_proof_escalation_comments.py --force --comment-dir <dir>`
...
- External proof follow-up can also run in GitHub Actions. Use
  ...
  with `python3 delivery_toolchain/e2e/check_external_proof_followup_workflow.py`. After
...
- Product closeout fleet pickup comments must track the current release target.
  ...
  `PANTHEON_STATUS_ROOT=/home/lupin/oday-plus python3 delivery_toolchain/e2e/sync_product_closeout_fleet_comment.py --release-sha "$(gh pr view 82 --json headRefOid --jq .headRefOid)" --apply`
  and then `python3 delivery_toolchain/e2e/check_product_closeout_fleet_notification.py`
...
- Release-owner fleet dispatch status must be checked as one live surface. Run
  `python3 delivery_toolchain/e2e/check_release_fleet_dispatch_status.py` after refreshing
...
- Product go/no-go external-proof wording must remain guarded. Run
  `python3 delivery_toolchain/e2e/check_product_go_no_go.py` before Human/Ops go/no-go so
...
- `python3 delivery_toolchain/e2e/check_external_proof_fleet_notifications.py` passes
...
- `python3 delivery_toolchain/e2e/check_product_closeout_fleet_notification.py` passes
  against live PR #82 comments for the current `headRefOid`.
- `python3 delivery_toolchain/e2e/check_release_fleet_dispatch_status.py` passes against
  live PR #82, #132-#138 issue handoff, and product closeout notification
  state.
- `python3 delivery_toolchain/e2e/check_product_go_no_go.py` passes against
  `docs/evidence/PRODUCT_RELEASE_GO_NO_GO.md`.
- `python3 delivery_toolchain/e2e/check_external_proof_handback_template.py` passes and
  accepted external proof artifacts cite the template fields.
- `python3 delivery_toolchain/e2e/check_external_proof_handback_status_board.py` passes
