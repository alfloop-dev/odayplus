...
- Remote staging rollout and production deployment are governed by
  `docs/deployment/EPHEMERAL_STAGING_PRODUCTION_ROLLOUT_PLAN.md` and the unified `Runtime Release` workflow.
- External provider activation and live provider proof require human approval receipts and per-source authorization before enabling live fetch.
- Release-owner fleet dispatch status is verified via standard toolchain gates.
