...
## Epoch 10 queue admission and failure ordering

Zero effective capacity now blocks actual queued launches, ready dispatch and helper admission. A previously healthy pool that joins a recovering auth account joins its single-canary fence; the auth transition and zero-capacity state survive stale disk saves. Provider failures now carry a durable microsecond logical clock and exact predecessor lineage, so same-second, identical-time and clock-backward failures cannot be replaced or cleared by older incident snapshots. Clearing the actual latest incident also retires its known legacy predecessors.

Regressions drive real process_queue and ready dispatch paths plus real producer/disk-save/clear interleavings, with independent auth and admitted-canary controls. The initial reproduction failed three cases. The first full run exposed a test fixture that mocked every account to zero but expected an independent account to run; its mock is now scoped to the affected account, preserving the original assertion. Original failing receipts are retained alongside the final complete passing run. Lint passed; the boundary inventory adds exactly two regression files and passes at 1161 files. Exact source hashes and original command/exit-code receipts are included. This source change still requires a newly authorized continuation and independent review; it does not deploy or alter the running supervisor.

## Failure clock continuity after formal clear

A supplementary audit reproduced a remaining case in a5ab3477: formal clear removes the active pause, so a subsequent failure after clock rollback could receive an epoch below the retained clearance and disappear on save. Legacy pauses without a worker ID exposed the same defect through the old timestamp matcher.

The producer now advances beyond retained matching provider/auth clearances as well as an active pause. A shared clearance-clock helper preserves modern exact epochs and derives the conservative legacy clock from both target and clear timestamps. The runtime and live matchers preserve a modern failure above that legacy clock. Existing targeted legacy clearance rules, provider/auth isolation and newer-failure fences remain in place.

Real producer/formal-clear/reload/stale-save regressions cover forward, identical and backward clocks, modern/legacy records, runless launch failures and independent provider/auth controls. Final focused suite: 179 passed plus 38 subtests; independent supplementary controls: 18 passed. Ruff and boundary (1161 files) passed. Original failing evidence, final commands/exit codes and source hashes are included. These are contributor verification receipts, not canonical reviewer approval or live deployment.
