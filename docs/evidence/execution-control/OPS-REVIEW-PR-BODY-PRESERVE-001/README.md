...
- **負責人**: Antigravity3 (Leased helper) / Antigravity6 (Canonical Owner)
- **評審人**: Codex2
...
1. **Review Bus 狀態同步與 Body 組合邏輯** (`.orchestrator/github_bus.py`):
   - 新增 `BUS_START_MARKER` 與 `BUS_END_MARKER` 常數。
   - 新增 `reconcile_pr_body(existing_body, bus_template_body)` 函式：
     - 若既有 PR body 為空，使用完整 Review Bus 範本。
     - 若既有 PR body 含有 `<!-- pantheon-bus -->` 與 `<!-- /pantheon-bus -->` 區塊，僅就地更新該區塊，完整保留區塊前後之自訂說明與筆記。
     - 若既有 PR body 含有舊版未閉合之標記，精確遷移並保留自訂尾部內容。
...
Code boundary checks passed for 984 files.
- archived: 14
- development_delivery_tooling: 66
- development_platform_system: 63
- evidence_artifact: 24
...
## 驗收條件對照表
- [x] **既有 PR body 有非模板內容時不得被 Review Bus 覆寫**: 透過 `reconcile_pr_body` 保留前綴、後綴或整段無標記之自訂內容，僅覆寫受標記之 Review Bus 區塊。
...
- [x] **加入現有 PR body preservation 與中文 template regression**: 於 `test_github_bus.py` 新增 9 個針對性回歸測試。
