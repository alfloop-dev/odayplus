...
- **記錄時間**: `2026-09-12T09:12:00Z`
- **續辦交付分支**: `task/ODP-DEV-ROLLOUT-001-RECOVERY-20260911`
- **目標基準 SHA (Target Base SHA)**: `4b35121031d054178550beaa2ca3e6ee8e0a39eb` (origin/dev tip composed)
...
2. **納入已完成 Manifest / Build / Candidate 原始證據**：
   ...
   - GitHub Actions workflow run `34179791241`（head SHA: `8c570a56353abdcc8ba70fe0a3fdd9b963902391`）已成功執行 `Build once and publish the immutable artifact handoff` job `101916404681`（conclusion: `success`），產出並簽名 4 項不可變映像；後續 deploy/lease/watch jobs 保持 `skipped`。這證明真實映像建置存在，但 live rollout 尚未在 GCP 執行。
...
4. **本次續辦交付成果**：
   ...
   - 建立至 `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` 之精確責任與一對多條款映射（A2 映射 A1/A2/A4，A4 映射 A7/A6，A5 映射 A8/A9）；
   - 完成 14 個節點、12 條邊之依賴圖有向無環 (DAG) 驗證與邊集 SHA256 綁定；
   - 更新下游補救任務未齊輸入清單，釐清 PR #1109 已合併與 sources-off 部署邊界；
...
| **Build-Once 工作流程** | GitHub Actions Run [`34179791241`](https://github.com/alfloop-dev/odayplus/actions/runs/34179791241) (head `8c570a56`) | `BUILD SUCCESS`<br>`DEPLOY SKIPPED` | Job `101916404681` 成功建置並簽名：<br>• `api`: `...@sha256:5e1a152e839cbfa7a2bf422b924928b56fad89f35e44243619a3f2fe98802cee`<br>• `web`: `...@sha256:38c716462b569b7420fe95788a84f8a1b3778e2e7c938d535a1dc13288a78971`<br>• `worker`: `...@sha256:b2c0e4473ad529ad71f215b76fc125115d8ba0b4e845a87e532d10ebdb8ddba3`<br>• `scheduler`: `...@sha256:f3fd22c00478d730273494c23c512a87c807de645a8b759d0af4908d82cd4cc9` |

> [!NOTE]
> Run 34179791241 證明了真實候選映像的 Build-Once 產出，但 Deploy 階段依准入控制保持 skipped。這是建置已完成的證據，非 rollout 通過。
...
| **A2** (所有元件符合 manifest digests) | **A1** (`驗證最新 authoritative manifest 之 candidate SHA、image digests、SBOM、Cosign 與 registry refs 均真實可解析`)、**A2** (`若 candidate 到 origin/dev 間含任何變更則建新 release 並重新 build once`)、及 **A4** (`live readback 顯示各元件綁定 exact digests`) | 佔位符 digest 由補救任務之真實 build-once image digests、SBOM 與 cosign 簽章驗證取代，並由 live readback 驗證一致性。 |
...
### 4.2 依賴關係比對與 DAG 無環驗證 (Dependency Graph & DAG Cycle Verification)
...
2. **Staging 依賴與生命週期處置說明 (Staging Lifecycle Disposition)**：
   - `ODP-EPHEMERAL-STAGING-ROLLOUT-001` 在 canonical 看板依賴 `ODP-DEV-ROLLOUT-001`。
   - 因本任務判定為 `false_done_superseded`，實質 dev rollout 由 `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` 承接。
   - 後續 staging 演練需引用真實 live digests 時，由其自身生命週期與准入閘門依賴 live remediation 產出，不在此任務手動變更看板依賴邊。
3. **DAG 無環檢查 (Cycle Verification)**：
   - 評估節點 (14 個)：
     `DPF-EMGI-LIVE-ROLLOUT-001`, `ODP-DEV-CANDIDATE-GATE-RECONCILIATION-002`, `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001`, `ODP-DEV-ROLLOUT-001`, `ODP-DEV-STAGED-GATE-RECONCILIATION-001`, `ODP-EPHEMERAL-STAGING-IAC-001`, `ODP-EPHEMERAL-STAGING-ROLLOUT-001`, `ODP-FIRST-RELEASE-ROLLBACK-RECOVERY-001`, `ODP-GITHUB-GCP-ENV-BOOTSTRAP-001`, `ODP-RELEASE-BUILD-HANDOFF-SNAPSHOT-ROLLBACK-WIRING-001`, `ODP-RELEASE-MANIFEST-LIVE-ARTIFACT-RECONCILE-001`, `ODP-RUNTIME-RELEASE-DISPATCH-CLI-INTEGRATION-001`, `ODP-RUNTIME-RELEASE-SINGLE-PATH-001`, `ODP-WEB-PASSWORD-FIRST-SECURITY-E2E-002`。
   - 完整邊集 (12 條邊)：
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-RELEASE-MANIFEST-LIVE-ARTIFACT-RECONCILE-001`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-RUNTIME-RELEASE-SINGLE-PATH-001`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-GITHUB-GCP-ENV-BOOTSTRAP-001`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `DPF-EMGI-LIVE-ROLLOUT-001`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-RELEASE-BUILD-HANDOFF-SNAPSHOT-ROLLBACK-WIRING-001`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-WEB-PASSWORD-FIRST-SECURITY-E2E-002`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-FIRST-RELEASE-ROLLBACK-RECOVERY-001`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-DEV-STAGED-GATE-RECONCILIATION-001`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-DEV-CANDIDATE-GATE-RECONCILIATION-002`
     - `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` ──► `ODP-RUNTIME-RELEASE-DISPATCH-CLI-INTEGRATION-001`
     - `ODP-EPHEMERAL-STAGING-ROLLOUT-001` ──► `ODP-EPHEMERAL-STAGING-IAC-001`
     - `ODP-EPHEMERAL-STAGING-ROLLOUT-001` ──► `ODP-DEV-ROLLOUT-001`
   - **邊集 SHA256**: `239a5050ae31ca80bb424c3c9a88a3c74c5c7b35eb5edefc5a1fc22aff229f10`
...
```bash
# 1. Git Diff 格式檢查
git diff --check 4b35121031d054178550beaa2ca3e6ee8e0a39eb HEAD

# 2. 結構化機讀 JSON 與 DAG 驗證
python3 -c '
import json
import hashlib
...
assert data["historical_delivery"]["delivered_files_total"] == 9
assert data["acceptance_criteria_summary"]["total_criteria"] == 5
assert data["acceptance_criteria_summary"]["by_historical_inventory_status"]["met_by_receipt"] == 1
assert data["acceptance_criteria_summary"]["by_historical_inventory_status"]["unmet_per_own_receipt"] == 2
assert data["acceptance_criteria_summary"]["by_historical_inventory_status"]["not_evidenced"] == 1
assert data["acceptance_criteria_summary"]["by_historical_inventory_status"]["partially_met"] == 1
...
assert len(data["criteria_to_remediation_task_mapping"]["mappings"]) == 5

# DAG cycle check
dag = data["dependency_graph_and_cycle_verification"]["dag_cycle_check"]
nodes = dag["evaluated_nodes"]
edges = dag["edge_set"]
...
canonical_edge_json = json.dumps(edges, sort_keys=True)
edge_hash = hashlib.sha256(canonical_edge_json.encode("utf-8")).hexdigest()
expected_hash = dag["edge_set_sha256"]
assert edge_hash == expected_hash, f"Edge hash mismatch: {edge_hash} vs {expected_hash}"

print("ODP-DEV-ROLLOUT-001 acceptance reconciliation validation: PASSED")
'
```

### 6.2 驗證執行結果 (Verification Execution Receipts)

| 驗證項目 | 執行命令 | 基準 SHA / 觀測時間 | 執行時長 | 退出碼 | 原始結果摘要 |
|---|---|---|---|---|---|
| **Git Diff 格式檢查** | `git diff --check 4b35121031d054178550beaa2ca3e6ee8e0a39eb HEAD` | `4b35121031d0`<br>`2026-09-12T09:12:30Z` | `0.02s` | `0` | 無 trailing whitespace 或 conflict marker，格式完全乾淨。 |
| **結構化 JSON 與 DAG 驗證** | `python3 -c '<acceptance reconciliation validation>'` | `HEAD`<br>`2026-09-12T09:12:35Z` | `0.05s` | `0` | `ODP-DEV-ROLLOUT-001 acceptance reconciliation validation: PASSED`（5 criteria、DAG 12 edges 無環確認、edge hash `239a5050…` 一致）。 |
| **PR 1109 Ancestry 驗證** | `gh pr view 1109` & `git merge-base --is-ancestor 640e35415aa33d5d53af21a8a527431b8f751cea HEAD` | `640e35415aa3`<br>`2026-09-12T09:11:04Z` | `0.45s` | `0` | PR #1109 merged at `2026-09-01T14:49:31Z`，已為 HEAD ancestor。 |
| **Run 34179791241 驗證** | `gh run view 34179791241 --repo alfloop-dev/odayplus` | `8c570a56353a`<br>`2026-09-12T09:11:08Z` | `0.52s` | `0` | workflow conclusion `success`，job `101916404681` build success，deploy/lease/watch skipped。 |
