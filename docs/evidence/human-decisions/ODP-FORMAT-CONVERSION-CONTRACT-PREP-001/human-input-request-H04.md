...
Repo 內查證事實（`origin/dev` @ `9048161e`，2026-09-08）：
...
| 項次 | 需要的資料 | 用途 | 備註 |
|------|-----------|------|------|
| 2.1.1 | 過去是否有門市進行過店型轉換（如一代店升級 G2、縮減為 G3 Compact）？ | 確認業務實際是否存在此流程 | 若從未發生過，需走正式需求修訂 pathway |
| 2.1.2 | 若有，請提供至少一筆歷史轉型案例的基本資料：門市代碼、原店型、新店型、改裝起迄日期 | 建立 migration seed 與測試 fixture | 可提供匿名化版本 |
| 2.1.3 | 未來是否有已規劃或預算編列的轉型專案？ | 確認需求時效性與資料可得時間 | 若無近期計畫，仍可完成契約但 Stage B 延後 |
...
| 2.3.5 | 改裝後爬坡期（Ramp）的預期長度與曲線形狀 | `ramp_months` 與 `ramp_curve_id` 定義 | 是否與新店開業 ramp 相同？若不同，如何不同？ |
...
- ramp_months 為 null 時，不得使用新店預設 ramp
...
若確認實體門市營運中從未有過且不規劃 Brownfield 轉型，請指示走以下路徑之一：

- **需求修訂（Amendment）**: 將 `FORMAT_CONVERSION` 自 `ODP-FR-SITE-001` 當前實作範圍移除，由授權人簽署
- **具期限豁免（Waiver）**: 暫時排除，設定複核日期

兩者均需要人類授權人簽署，AI 不可自行裁決。
