...
在 `ODP-DURABLE-PARTIAL-IMPL-001`（PR #1285 @ `b19513a1419497dce2ddd5054df5bbe0bd732a69`）合併交付後，程式庫內已具備真實可執行的批次 Worker (`handle_batch_listing_intake`)、預設 Worker 註冊 (`build_default_registry`)、逐項收據持久化 (`shared/jobs/receipts.py::derive_batch_status_and_summary`) 以及差異化重試 (`FAILED_ONLY`)。
...
| - 收據推導：shared/jobs/receipts.py::derive_batch_status_and_summary (推導 JobStatus.PARTIAL)       |
...
經由已合併之 PR #1285（HEAD commit `b19513a1419497dce2ddd5054df5bbe0bd732a69`），專案代碼庫中新增並落實以下生產者符號：

| 符號路徑 (Symbol Path) | 型態 | 實作責任 | 治理可解析性 |
|---|---|---|---|
| `apps/worker/oday_worker/handlers.py::handle_batch_listing_intake` | Function | 批次房源匯入處理器：執行三階段 checkpoint（嘗試開始、業務寫入、結果寫入），可產生 `JobStatus.PARTIAL` | `resolve()` 通過 (Exit 0) |
...
| `shared/jobs/receipts.py::derive_batch_status_and_summary` | Function | 純函式：自持久化 `ItemReceipt` 集合計算聚合狀態，於混合結果時確定推導為 `JobStatus.PARTIAL` | `resolve()` 通過 (Exit 0) |
...
2. **過時敘述清除與現況補充 (`member.note`)**：
   ...
   - 具名記錄 PR #1285（`b19513a1419497dce2ddd5054df5bbe0bd732a69`）交付之實體 Worker 與收據推導。
   - 保留 `HB-SHARED001-PARTIAL-001` 移交記錄與 Live 缺口說明。
3. **處置狀態與法定欄位 (`member.disposition`)**：
   - 狀態嚴格維持 `"BLOCKED_BY_EVIDENCE"`。
   - `history` 新增 `2026-09-11` 觀測記錄，保留既有 2026-09-03 與 2026-09-08 歷史條目。
...
| **下次檢視日期** | `2026-10-01` |
| **重啟觸發條件 (Reopen Trigger)** | (1) Live production queue/worker audit receipts demonstrate deployed jobs reporting PARTIAL in production runtime, OR (2) Human/Ops and Platform Infrastructure Lead formally execute H06 production sign-off and requirement closure. |
