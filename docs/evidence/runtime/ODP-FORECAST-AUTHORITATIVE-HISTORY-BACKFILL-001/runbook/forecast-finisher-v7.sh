...
log "finisher armed (v7: v6 gate unchanged; adds post-activation criterion-5 re-measurement)"
mkdir -p "$STAGE"
active=""
while true; do
...
if run_stage activate activation_receipt.json /tmp/odp-activate.out; then
  ...
  log "acceptance receipts captured; re-measuring criterion 5 against the activated target"
  run_criterion5_probe
else
