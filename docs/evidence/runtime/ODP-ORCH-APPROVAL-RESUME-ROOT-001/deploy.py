#!/usr/bin/env python3
"""Atomically deploy the reviewed cross-root approval-resolution fix.

Adapted from ODP-ORCH-ACTOR-REF-LIVE-ROLLOUT-001's driver, which published a
single file. This fix spans **three interdependent modules**:

    .orchestrator/common.py                     (adds the resolution primitives)
    .orchestrator/permission_broker.py          (imports them)
    .orchestrator/claude_permission_prompt_mcp.py (imports them)

``os.replace`` is atomic *per file*; a set of three renames is not. On a live
fleet a hook can start between two renames, so the on-disk set must be
import-consistent at **every instant**, not merely at the end. Two rules make
that true:

  1. **Stage-all-then-publish.** Every sibling for every target is written and
     fully verified before the first rename is issued. A verification failure
     anywhere means no rename happened anywhere.
  2. **Dependency order.** ``common.py`` publishes first. It only *adds*
     symbols, so the old broker keeps working against the new common; the new
     broker never runs against the old common. The observable sequence is
     old/old -> new/old (safe: superset) -> new/new. The reverse order would
     expose an ImportError window to any hook that fired inside it.

Per target the publish path is the reviewed one: same-directory ``O_CREAT|O_EXCL``
sibling, fsync, chmod to the target's existing mode, verify (sha256, length,
mode, byte-for-byte compare), only then ``os.replace`` + directory fsync. The
inode must change afterwards — that is the on-disk proof of rename-not-overwrite.

Nothing here starts, stops, restarts or reloads the Supervisor, and the run is
gated on that being observably true:

  * PREFLIGHT, before any target is touched: the unit must be loaded and
    active/running, MainPID must be a live process that still looks like the
    Supervisor, and ExecMainStartTimestamp / NRestarts must be readable.
  * CONTINUITY, after the last publish: still active/running, and MainPID,
    ExecMainStartTimestamp and NRestarts each *exactly* equal to preflight.
  * A failed or unparseable ``systemctl`` query is fatal in both positions.
    stderr is never discarded.

After a real publish each root also gets an **import smoke test**: the deployed
set must import and answer ``resolve_hook_config`` from that root's own
interpreter. A set that renames cleanly but cannot import is still a failure.

Usage::

    deploy.py --source-ref <commit> --backup-dir <dir> \
              --root /home/lupin/oday-plus --root /home/lupin/oday-plus-supervisor-live

``--corrupt-payload`` is the negative rehearsal: it flips one byte of one
payload *after* the digests are computed, so the staging gate must abort before
any rename is issued. Only ever point that at sandbox copies.
...
TASK_ID = "ODP-ORCH-APPROVAL-RESUME-ROOT-001"

# Publish order is load-bearing: a module must never be published before the
# module it imports from. common.py is additive, so new-common/old-broker is a
# safe intermediate state; old-common/new-broker is not.
TARGET_FILES = (
...
def without_deployed(lines: list[str]) -> list[str]:
    """Drop the files this driver publishes, so only *unrelated* dirt is compared."""
    deployed = {Path(p).as_posix() for p in TARGET_FILES}
    return [line for line in lines if not any(d in line for d in deployed)]
...
def continuity_failures(before: dict[str, str], after: dict[str, str]) -> list[str]:
    ...
    for field in CONTINUITY_FIELDS:
        if before[field] != after[field]:
            failures.append(f"{field} changed across the deploy: {before[field]!r} -> {after[field]!r}")
    return failures
...
def materialise(source_ref: str, rel_path: str) -> tuple[str, bytes, str]:
    """Read the exact reviewed bytes from the commit object, never the working tree."""
...
def stage(target: Path, payload: bytes, merged_sha: str, mode: int, corrupt: bool) -> tuple[Path, bool]:
    """Write and fully verify a same-directory sibling. Issues no rename."""
...
def import_smoke_test(root: Path) -> tuple[bool, str]:
    """The deployed set must actually import and expose the resolution entrypoint."""
    probe = (
        "import sys; sys.path.insert(0, '.');"
        "import common, permission_broker, claude_permission_prompt_mcp;"
        "assert callable(permission_broker.resolve_hook_config);"
        "assert callable(common.authoritative_status_root);"
        "assert callable(common.load_config_for_status_root);"
        "print('import-ok', common.STATUS_ROOT_ENV_VAR)"
    )
    # -B / PYTHONDONTWRITEBYTECODE: the smoke test must not leave a __pycache__
    # behind in a live root. Without it the driver dirties the very inventory it
    # then asserts is unchanged.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="sandbox rehearsal only: the unit gate is reported but not enforced",
    )
    ...
    print(f"# driver     : {Path(__file__).name} (stage-all -> verify-all -> ordered atomic publish)")
    print(f"# source ref : {args.source_ref}")
    print(f"# backups    : {backup_dir}")
    print(f"# order      : {' -> '.join(Path(p).name for p in TARGET_FILES)}")
    if args.corrupt_payload:
        print("# MODE       : NEGATIVE REHEARSAL — one payload is corrupted after hashing;")
        print("#              the staging gate must abort before any rename is issued.")
    print()
    ...
    # ---- Phase 1: stage and verify every sibling. No rename is issued here. ----
    ...
    if not all_staged_ok:
        ...
        for _root, target, sibling, before, _mode in staged:
            ...
            if after["sha256"] != before["sha256"] or after["inode"] != before["inode"]:
                failures.append(f"{target}: changed despite an aborted run")
        print()
    else:
        # ---- Phase 2: publish in dependency order, root by root. ----
        print("## phase 2 — ordered atomic publish (common.py first: it is a superset)")
    ...
    for root in roots:
        ...
        if not same_rest:
            ...
            only_after = set(without_deployed(dirty_after)) - set(without_deployed(dirty_before))
    ...
    if args.corrupt_payload:
        ...
        print(
            "RESULT: PASS — the staging gate aborted the corrupted payload and left"
    ...
    if failures:
        ...
        for item in failures:
            print(f"  - {item}")
        return 1
    print("RESULT: PASS — reviewed blobs published atomically, in dependency order, to every target")
    return 0
