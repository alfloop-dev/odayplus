...
- **候選 39ae43f6 部署前安全審查 (Predeployment Security Review — 本 task 執行)**:
  ...
  6. **候選 GCP IAM 最小權限離線審查 — 完整 Principal→Role→Resource→Condition/Necessity Inventory (iam.tf / main.tf / messaging.tf / cloud_run.tf)**:
     ...
     **未解決例外與必要性評估 (Necessity & Exception Assessment)**:
     ...
     3. `github_deployer` 之 `roles/cloudsql.client` (iam.tf:186) 為 project-level grant。必要性：migration 需要 Cloud SQL Auth Proxy 連線。收窄評估：GCP Cloud SQL IAM 政策不支援 per-instance 之 `roles/cloudsql.client` 綁定（僅支援專案層級），受 GCP API 限制無法進一步收窄。
     4. `runtime` 與 `web` SA 之 `roles/cloudsql.client` (iam.tf:25, iam.tf:64) 同為 project-level grant，受 GCP Cloud SQL IAM 架構僅支援專案層級之限制。
     - **結論**: 候選程式庫全數 IAM 宣告（`iam.tf`、`main.tf`、`messaging.tf`、`cloud_run.tf`）已完成完整 Principal→Role→Resource→Condition/Necessity 清冊審查。全專案未宣告任何 `roles/owner`、`roles/editor` 或 `roles/resourcemanager.organizationAdmin` 寬鬆角色。Deployer 之 project-level `run.admin` 與 `cloudscheduler.admin` 經確認為資源建立之生命週期營運必要性（非不可收窄之 API 限制，但為 initial provisioning 所需）；`cloudsql.client` 則受 Cloud SQL IAM 架構限制。WIF 限制部署身分僅能來自 `alfloop-dev/odayplus`。離線靜態審查結論為通過 (PASSED)，上述營運必要性與例外已明確記錄。Live IAM state readback（確認 Terraform 宣告與 GCP 實際環境一致）屬部署期驗證，由 `ODP-DEV-LIVE-ROLLOUT-REMEDIATION-001` (Owner `Antigravity2`) 執行。
   ...
   8. **NOTICE/SBOM/D05 個別對帳 — Sec-6 精確測試引用與 committed SBOM vs container attestation 區分**:
      ...
      - **Bounded purl inspection — 抽樣查核完成**: 針對 candidate `39ae43f6fe679f03dd7df459a51835cbd2d54f77` 之 committed `docs/evidence/sbom.json` (CycloneDX 1.5, 767 components) 執行有界 purl 格式與路徑檢查（執行命令：`python3 -c "import json; sbom=json.load(open('docs/evidence/sbom.json')); comps=sbom.get('components',[]); assert len(comps)==767; assert all(c.get('purl','').startswith('pkg:') and 'node_modules' not in c.get('purl','') and 'node_modules' not in c.get('name','') for c in comps)"`，exit 0, duration 0.062s at 2026-09-20T14:29:03Z）。查核 predicate 明確排除 malformed nested `node_modules` 名稱與路徑：767/767 個 component 均具備有效 `pkg:` 前綴且 0 個包含 nested `node_modules`。結合 CI 測試 `test_sbom_licenses_purls_suppliers_hashes_populated`（驗證 purl 前綴 `pkg:`、version、licenses、scope），歷史 14 個 malformed nested purls 缺陷於候選 `39ae43f6` 已確認修復。
