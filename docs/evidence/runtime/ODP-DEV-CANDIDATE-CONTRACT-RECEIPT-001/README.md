...
- **驗證結果**:
  - `delivery_toolchain/openapi/check_drift.py --skip-diff` 在 candidate C 上執行通過 (EXIT=0)，證明 `openapi.json` 與 live FastAPI app 路由完全吻合，且 `types.ts` 與 `openapi.json` 位元組一致。
  - `tests/contract/test_openapi_artifact_and_client.py` 17 項測試全部通過 (EXIT=0)，驗證了錯誤封套 (ErrorEnvelope)、版本化路徑、輸出確定性與 diff 分類演算法。

### 2.2 Event Schema 相容性
- **範圍**: `packages/schemas/` 內各 domain intake、event 與 canonical schemas。
- **驗證結果**:
  - `tests/contract/test_assisted_listing_intake_events.py`、`test_assisted_listing_intake_schema.py`、`test_assisted_listing_intake_states.py`、`test_canonical_schema.py`、`test_ingestion_contracts.py`、`test_signal_store_client.py` 等契約測試在 candidate C 上均通過。
  - Intake 事件與狀態機生命週期符合 JSON Schema 規格。
...
- **驗證結果**:
  - `tests/contract/test_oday_data_contract_pin.py` (30 tests) 與 `test_oday_data_product_contract_pin.py` (47 tests) 全部通過 (EXIT=0)，證明消費者模式由已發布的 release bundle 生成，完全無直連 producer 內部 DDL/catalog 之依賴。

### 2.4 模型與決策介面 I/O 相容性
- **範圍**: 定價模擬 (Pricing Simulation)、區域分析 (Heatzone Composition)、決策政策登錄 (Decision Policy Registry) 與評分 API。
- **驗證結果**:
  - `tests/contract/test_pricing_simulation_contract.py`、`test_heatzone_composition_schema.py`、`test_decision_policy_registry_schema.py`、`test_operator_network_scoring_api.py` 等測試在 candidate C 上通過 (45 passed, EXIT=0)。
  - 介面封套、參數型別與 writeback idempotency 皆受契約保護。
...
- **Expand/Contract 規範**: 依據《EPHEMERAL_STAGING_PRODUCTION_ROLLOUT_PLAN.md》§8.4，系統不採破壞性 down migration，contract migration 延後至相容觀察期結束後獨立 release。
...
1. **無已核准前版基線**: Candidate C 是對 dev target 的首次 release (`initial_release_recovery`)，在 dev/staging/prod 環境皆不存在上一已核准的 release/manifest。
2. **禁止自設 C 為基線**: 依 acceptance criteria 規範，嚴禁以 C 與 C 比對（會虛假消除所有 breaking changes）。
3. **禁止以移動中的 dev 為基線**: 嚴禁以持續前進的 `origin/dev` 作為基線。
4. **結論**: 在缺乏可信 immutable baseline 的情況下，無法宣告與前版的相容性證明；這項缺口維持保留為 `unknown/blocked`，不偽造相容結論。

---

## 4. 離線驗證執行紀錄 (Offline Verification Receipts)

所有驗證均在隔離 scratch (`HEAD = 596b9c9a1788d952811a2bf8d4bba8a4e4d76b12`) 下使用 Python 3.12 執行：

| 執行編號 | 驗證命令 | 執行時間 (s) | Exit Code | 結果摘要 |
|---|---|---|---|---|
| `EXEC-01` | `uv run --python 3.12 python3 delivery_toolchain/openapi/check_drift.py --skip-diff` | 18.2s | `0` | OpenAPI artifact 與 generated client freshness 通過 |
| `EXEC-02` | `uv run --python 3.12 pytest tests/contract/test_openapi_artifact_and_client.py tests/contract/test_oday_data_contract_pin.py tests/contract/test_oday_data_product_contract_pin.py` | 64.85s | `0` | 94 passed (OpenAPI 17, Data Pin 30, Product Pin 47) |
| `EXEC-03` | `uv run --python 3.12 pytest tests/contract/test_pricing_simulation_contract.py tests/contract/test_heatzone_composition_schema.py tests/contract/test_decision_policy_registry_schema.py tests/contract/test_canonical_schema.py` | 33.51s | `0` | 45 passed (Pricing, Heatzone, Decision Policy, Canonical Schema) |
...
2. `review-receipt.json`: 機器可讀之 Gate 1 Contract Review 收據（含候選 C 綁定、五項檢查詳細狀態、離線驗證紀錄與 blocker 清單）。
3. `criteria-evidence-matrix.json`: Gate 1 驗收準則與證據對應矩陣（含檔案路徑、SHA-256、測試模組與負責 owner）。
...
| `BLK-G1-03` | `RELEASE_GATE_REGISTRY.json` 中 Gate 1 維持 blocked，fail-closed NO-GO 決定不變。 | Human/Ops | Blocked (Fail-Closed) |
