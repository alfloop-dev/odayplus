...
十九項待修（第二十項判不了），分成七批：第 0 批是前置資料確認，第 1–6 批是執行批次。排序依「錯數字離人的決定有多近」，但**執行順序不等於危害順序**——第 0 批必須先做，因為它是第 6 批部分項目的前提。
...
| 6 | 其餘缺席（含第 0 批確認後仍適用的三項） | 不產生錯答案，可長期排；第 0 批確認資料存在後，才把那三項排入實作 | 第 0 批決定 `SITE-001`／`NET-002`；其餘無 |
...
| 項目 | 做法要點 |
|---|---|
| `SITE-001` Brand Transfer | 第 0 批確認資料源存在後才進入實作；若不存在，記為 `decided-not-doing` 並留下理由 |
| `SITE-001` Format Conversion | 第 0 批確認這個業務動作確實發生且有資料後才進入實作；若不存在，記為 `decided-not-doing` 並留下理由 |
| `NET-002` 租約條件限制（檔期、解約金） | 第 0 批確認資料源存在後才評估實作；不包含已裁決不做的 per-period 時序限制與完整稀釋配對形式 |
