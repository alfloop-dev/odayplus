...
def expand_forecast_horizon_rows(
    ...
    """Create one training row per origin and canonical forecast horizon.

    A daily source row at date ``D`` carries features computed only from dates
    before ``D``. Its horizon label is the mean daily net revenue over
    ``D..D+(7*horizon_weeks)-1``. A row is emitted only when the complete future
    window is present and contiguous.
    """
