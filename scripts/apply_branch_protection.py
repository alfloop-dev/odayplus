...
`dev` before it is even allowed to enter the queue. `branch_overrides` in the
...
def branch_override(policy: dict, branch: str) -> dict:
    overrides = policy.get("branch_overrides") or {}
    value = overrides.get(branch) or {}
    return value if isinstance(value, dict) else {}


def strict_for_branch(policy: dict, branch: str) -> bool:
    override = branch_override(policy, branch)
    if "strict" in override:
        return bool(override["strict"])
    return bool(policy.get("strict", True))


def build_payload(policy: dict, branch: str) -> dict:
    # Transform policy.json to standard GitHub API payload format
    payload = {
        "required_status_checks": {
            "strict": strict_for_branch(policy, branch),
...
def merge_queue_config(policy: dict, branch: str) -> dict | None:
    queues = policy.get("merge_queue") or {}
...
def main(argv: list[str] | None = None) -> int:
    ...
    policy = load_policy(ROOT / ".github/branch-protection/policy.json")
    if policy is None:
        return 1
    ...
    for branch in BRANCHES:
        ...
        payload = build_payload(policy, branch)
        if rollback:
